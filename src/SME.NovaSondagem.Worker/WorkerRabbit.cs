...
using static SME.NovaSondagem.Infra.Services.ServicoLog;
...
namespace SME.NovaSondagem.Worker
{
    public class WorkerRabbit : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await channel.ExchangeDeclareAsync(ExchangeRabbit.NovaSondagem, ExchangeType.Direct, true);
            await channel.ExchangeDeclareAsync(ExchangeRabbit.NovaSondagemDeadLetter, ExchangeType.Direct, true);
        ...
        private async Task DeclararFilasAsync(IChannel channel)
        {
            foreach (var fila in typeof(RotasRabbit).ObterConstantesPublicas<string>())
            {
                ...
                await channel.QueueBindAsync(fila, ExchangeRabbit.NovaSondagem, fila, null);
                ...
                await channel.QueueBindAsync(filaDeadLetterFinal, ExchangeRabbit.NovaSondagemDeadLetter, filaDeadLetterFinal, null);
            }
        }
        ...
        private Dictionary<string, object> ObterArgumentoDaFilaDeadLetter(string fila)
        {
            ...
            var ttl = comandos.ContainsKey(fila) ? comandos[fila].Ttl : ExchangeRabbit.NovaSondagemDeadLetterTtl;
                ...
                catch (Exception ex)
                {
                    ...
                    if (++rejeicoes >= comandoRabbit.QuantidadeReprocessamentoDeadLetter)
                    {
                        ...
                        await servicoMensageria.Publicar(mensagemRabbit, filaFinal,
                            ExchangeRabbit.NovaSondagemDeadLetter,
                            "PublicarDeadLetter");
                    }
                    else
                    {
